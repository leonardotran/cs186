...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int keyIndex = InnerNode.numLessThanEqual(key, keys);
        BPlusNode childNode = getChild(keyIndex);
        Optional<Pair<DataBox, Long>> newEntry = childNode.put(key, rid);
        if (newEntry.isPresent()) {
            if (keyIndex >= keys.size()) {
                keys.add(newEntry.get().getFirst());
            } else {
                keys.add(keyIndex, newEntry.get().getFirst());
            }
            if (keyIndex + 1 >= children.size()) {
                children.add(newEntry.get().getSecond());
            } else {
                children.add(keyIndex + 1, newEntry.get().getSecond());
            }

            if (keys.size() > 2*d) {
                List<DataBox> newKeys = new ArrayList<>();
                List<Long> newChildren = new ArrayList<>();
                DataBox splitKey = keys.remove(d);
                for (int i = 0; i < d; i++) {
                    newKeys.add(0, keys.remove(keys.size() - 1));
                }
                for (int i = 0; i <= d; i++) {
                    newChildren.add(0, children.remove(children.size() - 1));
                }
                InnerNode newInner = new InnerNode(metadata, bufferManager, newKeys, newChildren, treeContext);
                long newNodePageNum = newInner.getPage().getPageNum();
                sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        Optional<Pair<DataBox, Long>> newEntry;
        while (keyIndex <= 2*d && data.hasNext()) {
            BPlusNode rightmostNode;
            rightmostNode = getChild(keyIndex);

            newEntry = rightmostNode.bulkLoad(data, fillFactor);
           if (newEntry.isPresent()) {
               keys.add(newEntry.get().getFirst());
               children.add(newEntry.get().getSecond());
               keyIndex++;
