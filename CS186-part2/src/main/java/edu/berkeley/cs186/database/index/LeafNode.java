...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.contains(key)) {
            throw new BPlusTreeException("B+ trees do not support duplicate entries with the same key");
        }
        ...
        if (keys.size() > 2*d) {
            ...
            for (int i = 0; i < d + 1; i++) {
                ...
                newRids.add(0, rids.remove(rids.size() - 1));
            }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement
        int numRecords = (int) Math.ceil(2*metadata.getOrder() * fillFactor);
        int recordIndex = keys.size();
        while (recordIndex < numRecords && data.hasNext()) {
        ...
        if (data.hasNext()) {
            ...
            List<RecordId> newRids = new ArrayList<>();
            Pair<DataBox, RecordId> nextRecord = data.next();
            newKeys.add(nextRecord.getFirst());
            newRids.add(nextRecord.getSecond());
            LeafNode newLeaf = new LeafNode(metadata, bufferManager, newKeys, newRids, rightSibling, treeContext);
            long newNodePageNum = newLeaf.getPage().getPageNum();
            rightSibling = Optional.of(newNodePageNum);
            sync();
    ...
    public void remove(DataBox key) {
        // TODO(proj2): implement
        int keyIndex = InnerNode.numLessThanEqual(key, keys) - 1;
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Long rightSiblingIdPID = buf.getLong();
        Optional<Long> rightSibling;
        if (rightSiblingIdPID == -1L) {
            rightSibling = Optional.empty();
        } else {
            rightSibling = Optional.of(rightSiblingIdPID);
        }
